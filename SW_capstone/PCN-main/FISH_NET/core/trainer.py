...
class Trainer():
    ...
    def _train_epoch(self):
        ...
        mae = 0
        for images, gt, img_name, _ in self.train_loader:
            self.iteration += 1
            ...
            if self.iteration * self.config['trainer']['batch_size'] % 40000 == 0: # visualization per epoch
                orig_imgs = postprocess(images)
                gt_imgs = postprocess(gt)
                pred_imgs = postprocess(pred_img)
                for i in range(len(orig_imgs)):
                    Image.fromarray(pred_imgs[i]).save(os.path.join('/content/drive/MyDrive/SW_capstone/PCN-main/FISH_Net/release_model/', str(self.iteration), '/{}_pred.png'.format(img_name[i].split('.')[0])))
                    Image.fromarray(orig_imgs[i]).save(os.path.join('/content/drive/MyDrive/SW_capstone/PCN-main/FISH_Net/release_model/', str(self.iteration), '/{}_orig.png'.format(img_name[i].split('.')[0])))
                    Image.fromarray(gt_imgs[i]).save(os.path.join('/content/drive/MyDrive/SW_capstone/PCN-main/FISH_Net/release_model/', str(self.iteration), '/{}_gt.png'.format(img_name[i].split('.')[0])))
            ...
            logs = [("epoch", self.epoch), ("iter", self.iteration), ("lr", self.get_lr()),
                    ('mae', mae), ('gen_loss', gen_fake_loss.item()),('dis_loss', dis_fake_loss.item()), 
